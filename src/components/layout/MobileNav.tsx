...
export function MobileNav({ currentLessonId, currentLevel, onSelectLesson, showLessons, onToggleLessons }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {/* Slide-up lesson drawer */}
      {showLessons && (
        <div className="fixed inset-x-0 bottom-0 z-50 animate-slide-up">
          <div className="bg-bg-card rounded-t-2xl max-h-[70dvh] flex flex-col" style={{ boxShadow: 'var(--shadow-float)' }}>
            {/* Drawer handle */}
            ...
            <div className="px-5 pb-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={onToggleLessons}
                  className="w-8 h-8 flex items-center justify-center rounded-lg text-text-muted hover:bg-bg-secondary transition-colors"
                >
              ...
              {levels.map((level) => (
                <div key={level.id} className="mb-3">
                  <div className="flex items-center gap-2 px-2 py-1.5">
                    <span className="w-5 h-5 rounded-md bg-lavender text-white text-[10px] font-bold flex items-center justify-center">
                      {level.id}
                    </span>
                    <span className="text-xs font-semibold text-text-secondary">{level.title}</span>
                  </div>
                    ...
                    return (
                      <button
                        ...
                        className={`
                          ...
                          ${isCurrent ? 'bg-lavender-light text-lavender font-semibold' : ''}
                          ${isDone && !isCurrent ? 'text-text-secondary' : ''}
                          ${!canAccess ? 'text-text-muted/40 cursor-not-allowed' : 'active:scale-[0.98]'}
                          ${!isCurrent && !isDone && canAccess ? 'text-text-primary' : ''}
                        `}
                      >
                        <span className="flex-shrink-0">
                          {isDone ? (
                            <span className="w-6 h-6 rounded-full bg-mint-light flex items-center justify-center">
      ...
      {/* Bottom bar */}
      <div className="bg-bg-card border-t border-border safe-bottom" style={{ boxShadow: '0 -1px 8px rgba(30, 27, 46, 0.04)' }}>
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onToggleLessons}
              className="w-9 h-9 rounded-xl bg-bg-secondary flex items-center justify-center active:scale-95 transition-transform"
            >
